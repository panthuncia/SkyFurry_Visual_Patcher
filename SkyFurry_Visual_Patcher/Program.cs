...
namespace SkyFurry_Visual_Patcher {
    public class Program {
        ...
        private static void patchVisuals(IPatcherState<ISkyrimMod, ISkyrimModGetter> state) {
            ...
            (List<String> modNames, List<ISkyrimModGetter> modsToPatch) = state.LoadOrder.getModsFromMaster(_settings.Value.SkyFurryBaseModName, SkyFurry);
        ...
        private static void patchFlowingFur(IPatcherState<ISkyrimMod, ISkyrimModGetter> state) {
            ...
            if (FlowingFur != null) {
                ...
                (List<String> modNames, List<ISkyrimModGetter> modsToPatch) = state.LoadOrder.getModsFromMaster(_settings.Value.FlowingFurModName, FlowingFur);
                int ModIndex = -1;
                foreach (ISkyrimModGetter mod in modsToPatch) {
                    int processed = 0;
                    int ignored = 0;
        ...
        private static void patchSharpClaws(IPatcherState<ISkyrimMod, ISkyrimModGetter> state) {
            ...
            if (sharpClaws != null) {
                ...
                //patch races
                (List<String> modNames, List<ISkyrimModGetter> modsToPatch) = state.LoadOrder.getModsFromMaster(_settings.Value.SharpClawsModName, sharpClaws);
                foreach (ISkyrimModGetter mod in modsToPatch) {
